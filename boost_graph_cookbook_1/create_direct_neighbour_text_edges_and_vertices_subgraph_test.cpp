...
BOOST_AUTO_TEST_CASE(create_direct_neighbour_text_edges_and_vertices_subgraph_though)
{
  //K2
  {
    ...
    for (auto i=vip.first; i!=j; ++i) {
      const auto h = create_direct_neighbour_text_edges_and_vertices_subgraph(
        *i,g
      );
      BOOST_CHECK(boost::num_vertices(h) == 2);
      BOOST_CHECK(boost::num_edges(h) == 1);
      const auto vertex_texts = get_vertex_texts(h);
      std::set<std::string> vertex_texts_set(std::begin(vertex_texts),std::end(vertex_texts));
      BOOST_CHECK(vertex_texts_set.count("x") == 1);
      BOOST_CHECK(vertex_texts_set.count("y") == 1);
      const auto edge_texts = get_edge_texts(h);
      std::set<std::string> edge_texts_set(std::begin(edge_texts),std::end(edge_texts));

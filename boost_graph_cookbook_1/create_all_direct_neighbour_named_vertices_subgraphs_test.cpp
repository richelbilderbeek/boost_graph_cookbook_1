...
BOOST_AUTO_TEST_CASE(
  create_all_direct_neighbour_named_vertices_subgraphs_thorough)
{
  // K2
  {
  ...
  // Path
  {
    ...
    for (int i = 0; i != sz; ++i) {
      ...
      const std::string base_filename{
        "create_all_direct_neighbour_named_vertices_subgraphs_"
        + std::to_string(i)
      };
  ...
  {
    const auto v = create_all_direct_neighbour_named_vertices_subgraphs(
      create_named_vertices_petersen_graph());

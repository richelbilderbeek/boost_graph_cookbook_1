...
#include "create_direct_neighbour_custom_vertices_subgraph_demo.impl"
...
BOOST_AUTO_TEST_CASE(create_direct_neighbour_custom_vertices_subgraph_thorough)
{
  // K2
  {
    ...
    for (auto i = vip.first; i != j; ++i) {
      const auto h = create_direct_neighbour_custom_vertices_subgraph(*i, g);
  ...
  // Path graph
  {
    ...
    const auto g = create_custom_vertices_path_graph({ a, b, c });
    const auto vip = vertices(g);
    const auto j = vip.second;
    for (auto i = vip.first; i != j; ++i) {

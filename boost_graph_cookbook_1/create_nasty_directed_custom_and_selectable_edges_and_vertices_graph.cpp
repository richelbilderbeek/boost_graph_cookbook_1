...
create_nasty_directed_custom_and_selectable_edges_and_vertices_graph()
{
  ...
  for (int i = 0; i != 3; ++i) // Duplicate labels
  {
    ...
    add_custom_and_selectable_edge(
      my_custom_edge("A", "BCDEFGHIJKLMN"), i % 2, g);
    // Spaces
    add_custom_and_selectable_edge(my_custom_edge("A B", " C D"), i % 2, g);
    // Spaces in end and front
    add_custom_and_selectable_edge(
      my_custom_edge(" A B ", "  C  D  "), i % 2, g);
    // Underscores

...
BOOST_AUTO_TEST_CASE(create_direct_neighbour_bundled_edges_and_vertices_subgraph_thorough)
{
  //K2
  {
  ...
  //Path graph
  {
    ...
    const auto g = create_bundled_edges_and_vertices_path_graph(my_edges, my_vertices);
    const auto vip = vertices(g);
    const auto j = vip.second;
    for (auto i=vip.first; i!=j; ++i) {

...
BOOST_AUTO_TEST_CASE(test_add_bundled_edge_between_vertices_for_another_edge)
{
  ...
  using another_graph = boost::adjacency_list<boost::vecS, boost::vecS,
    boost::undirectedS, another_vertex, another_edge>;
  another_graph g;
  const auto from = add_bundled_vertex(another_vertex("Bert"), g);
  const auto to = add_bundled_vertex(another_vertex("Ernie"), g);
  add_bundled_edge_between_vertices(another_edge("Friends"), from, to, g);
  BOOST_CHECK(boost::num_edges(g) == 1);
}

...
BOOST_AUTO_TEST_CASE(test_add_bundled_edge_one_edge)
{
...
BOOST_AUTO_TEST_CASE(test_add_bundled_edge_two_edges)
{
  auto g = create_empty_undirected_bundled_edges_and_vertices_graph();
  BOOST_CHECK(boost::num_vertices(g) == 0);
  BOOST_CHECK(boost::num_edges(g) == 0);
...
BOOST_AUTO_TEST_CASE(test_add_bundled_edge_for_another_edge_type)
{
  ...
  BOOST_CHECK(boost::num_vertices(g) == 0);
  BOOST_CHECK(boost::num_edges(g) == 0);
  const another_bundled_edge edge("X");
  const auto vd_from = add_bundled_vertex(another_bundled_vertex("From"), g);
  const auto vd_to = add_bundled_vertex(another_bundled_vertex("To"), g);
  add_bundled_edge(vd_from, vd_to, edge, g);
  BOOST_CHECK(boost::num_vertices(g) == 2);
  BOOST_CHECK(boost::num_edges(g) == 1);
}

...
BOOST_AUTO_TEST_CASE(create_text_vertices_markov_chain_save_and_load)
{
  //Nasty graph
  {
    ...
    save_text_vertices_graph_to_dot(g, filename);
    const auto h = load_directed_text_vertices_graph_from_dot(filename);
  ...
  //Create the .dot and .svg of the 'create_text_vertices_markov_chain' chapter
  {
    ...
    const std::string dot_filename{"create_text_vertices_markov_chain.dot"};
    const std::string svg_filename{"create_text_vertices_markov_chain.svg"};

...
my_custom_edge::my_custom_edge(const std::string& name,
  const std::string& description, const double width, const double height)
  : m_name{ name }
  ...
  , m_height{ height }
{
  ...
  if (graphviz_decode(graphviz_encode(m_description)) != m_description) {
    std::stringstream msg;
    msg << __func__
        << ": cannot successfully encode and decode the description '"
...
const std::string& my_custom_edge::get_name() const noexcept { return m_name; }

double my_custom_edge::get_width() const noexcept { return m_width; }

double my_custom_edge::get_height() const noexcept { return m_height; }

bool operator==(const my_custom_edge& lhs, const my_custom_edge& rhs) noexcept
{
  return lhs.get_description() == rhs.get_description()
    && lhs.get_name() == rhs.get_name() && lhs.get_width() == rhs.get_width()
...
{
  os << graphviz_encode(v.get_name()) << ","
     << graphviz_encode(v.get_description()) << "," << v.get_width() << ","
     << v.get_height();
  return os;
}
...
std::istream& operator>>(std::istream& is, my_custom_edge& the_edge)
{
  ...
  if (w.size() != 4) {
    std::stringstream msg;
    msg << __func__ << ": "
        << "invalid line with " << w.size() << " instead of four elements";
    throw std::runtime_error(msg.str());
  }
  the_edge = my_custom_edge(graphviz_decode(w[0]), graphviz_decode(w[1]),
    boost::lexical_cast<double>(w[2]), boost::lexical_cast<double>(w[3]));
  return is;
}

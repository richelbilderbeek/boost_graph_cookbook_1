...
BOOST_AUTO_TEST_CASE(
  load_directed_bundled_edges_and_vertices_graph_from_dot_thorough)
{
  ...
  // Create the picture
  // 'load_directed_bundled_edges_and_vertices_graph_from_dot_test_markov_chain.svg'
  ...
  // to .svg
  {
    ...
    const auto h
      = load_directed_bundled_edges_and_vertices_graph_from_dot(dot_filename);
    BOOST_CHECK(boost::num_edges(g) == boost::num_edges(h));
    BOOST_CHECK(boost::num_vertices(g) == boost::num_vertices(h));
  ...
  // Load incorrect .dot files: write an undirected graph
  {
    try {
      ...
      BOOST_CHECK(std::string(e.what())
        == "read_graphviz: Tried to read an undirected graph into a directed "
           "graph.");
    }
  }
}
...
BOOST_AUTO_TEST_CASE(
  load_directed_bundled_edges_and_vertices_graph_from_dot_when_file_is_absent)
{
  const std::string dot_filename{ "load_directed_bundled_edges_and_vertices_"

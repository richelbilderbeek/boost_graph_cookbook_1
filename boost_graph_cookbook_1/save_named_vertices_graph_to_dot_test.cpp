...
BOOST_AUTO_TEST_CASE(save_named_vertices_graph_to_dot_thorough)
{
  ...
  // Show it does store the vertex names
  {
    ...
    const std::string base_filename{
      "save_named_vertices_graph_to_dot_test_named_vertices_k2_graph"
    };
    const std::string dot_filename{ base_filename + ".dot" };
    save_named_vertices_graph_to_dot(g, dot_filename);
  ...
  // Show it does not store the edges' names
  {
    ...
    const std::string base_filename{ "save_named_vertices_graph_to_dot_test_"
                                     "named_edges_and_vertices_k3_graph" };
    const std::string dot_filename{ base_filename + ".dot" };
    save_named_vertices_graph_to_dot(g, dot_filename);
    const std::vector<std::string> text{ file_to_vector(dot_filename) };
    BOOST_CHECK(!text.empty());

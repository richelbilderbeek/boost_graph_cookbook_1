...
BOOST_AUTO_TEST_CASE(
  create_custom_and_selectable_vertices_path_graph_no_vertices)
{
...
BOOST_AUTO_TEST_CASE(
  create_custom_and_selectable_vertices_path_graph_one_vertex)
{
...
BOOST_AUTO_TEST_CASE(
  create_custom_and_selectable_vertices_path_graph_one_vertex_too_few)
{
  ...
  BOOST_CHECK_THROW(create_custom_and_selectable_vertices_path_graph(
                      my_custom_vertexes, my_selectednesses),
...
BOOST_AUTO_TEST_CASE(create_custom_and_selectable_vertices_path_graph_two_edges)
{
  ...
  const auto g = create_custom_and_selectable_vertices_path_graph(
    my_custom_vertexes, { false, true, false });
...
BOOST_AUTO_TEST_CASE(
  create_custom_and_selectable_vertices_path_graph_create_pictures)
{
  // Create the .dot and .svg of the
  // 'create_custom_and_selectable_vertices_path_graph' chapter for (const int n:
  // {3,4, 5} )
  ...
  const auto g = create_custom_and_selectable_vertices_path_graph(
    my_custom_vertexes, { false, true, false, false });
  const std::string base_filename{
    "create_custom_and_selectable_vertices_path_graph_4"
  };

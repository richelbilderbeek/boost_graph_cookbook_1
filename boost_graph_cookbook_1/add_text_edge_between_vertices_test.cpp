...
BOOST_AUTO_TEST_CASE(add_text_edge_between_vertices_add_another)
{
  ...
  const auto from = add_text_vertex(vertex_name1, g);
  const auto to = add_text_vertex(vertex_name2, g);
  add_text_edge_between_vertices(edge_name, from, to, g);
  BOOST_CHECK(boost::num_edges(g) == 1);
}

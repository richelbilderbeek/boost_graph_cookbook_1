...
BOOST_AUTO_TEST_CASE(load_directed_text_vertices_graph_from_dot_thorough)
{
  //Basic
  {
    ...
    const std::string dot_filename{"load_directed_text_vertices_graph_from_dot_test_markov_chain.dot"};
    save_text_vertices_graph_to_dot(g, dot_filename);
    const auto h = load_directed_text_vertices_graph_from_dot(dot_filename);
    save_text_vertices_graph_to_dot(h, dot_filename);
    BOOST_CHECK(get_vertex_names(g) == get_vertex_names(h));
  }
  //Nasty
  {
    const auto g = create_nasty_directed_text_vertices_graph();
    const std::string dot_filename{"create_nasty_directed_text_vertices_graph.dot"};
    save_text_vertices_graph_to_dot(g, dot_filename);
    const auto h = load_directed_text_vertices_graph_from_dot(dot_filename);
    save_text_vertices_graph_to_dot(h, dot_filename);
...
BOOST_AUTO_TEST_CASE(load_directed_text_vertices_graph_from_dot_when_file_is_absent)
{
  ...
  BOOST_CHECK_THROW(
    load_directed_text_vertices_graph_from_dot(dot_filename),

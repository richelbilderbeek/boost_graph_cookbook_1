...
BOOST_AUTO_TEST_CASE(
  create_direct_neighbour_subgraph_including_in_edges_thorough)
{
  // a -> b
  {
    ...
    {
      const auto h
        = create_direct_neighbour_subgraph_including_in_edges(vd_a, g);
      BOOST_CHECK(boost::num_vertices(h) == 2);
      BOOST_CHECK(boost::num_edges(h) == 1);
    }
    // b is connected to a by in edge, create_direct_neighbour_subgraph does not
    // include a
    {

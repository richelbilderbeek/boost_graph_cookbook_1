...
#include "remove_selected_custom_edges_and_vertices_demo.impl"
...
BOOST_AUTO_TEST_CASE(
  remove_selected_custom_edges_and_vertices_remove_one_edge_and_one_node)
{
  ...
  const auto g = create_custom_and_selectable_edges_and_vertices_path_graph(
    my_custom_and_selectable_edges, { true }, my_custom_vertexes,
    { false, true });
  ...
  BOOST_CHECK_EQUAL(boost::num_edges(h), 0);
  BOOST_CHECK_EQUAL(boost::num_vertices(h), 1);
}

BOOST_AUTO_TEST_CASE(remove_selected_custom_edges_and_vertices_remove_no_edge)
{
  const std::vector<my_custom_vertex> my_custom_vertexes
    = { my_custom_vertex("X"), my_custom_vertex("Y") };
  const std::vector<my_custom_edge> my_custom_and_selectable_edges

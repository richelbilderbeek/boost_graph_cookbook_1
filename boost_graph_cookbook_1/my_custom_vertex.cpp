...
my_custom_vertex::my_custom_vertex(const std::string& name,
  const std::string& description, const double x, const double y)
  : m_name{ name }
...
double my_custom_vertex::get_x() const noexcept { return m_x; }

double my_custom_vertex::get_y() const noexcept { return m_y; }

bool operator==(
  const my_custom_vertex& lhs, const my_custom_vertex& rhs) noexcept
{
  return lhs.get_description() == rhs.get_description()
    && lhs.get_name() == rhs.get_name() && lhs.get_x() == rhs.get_x()
    && lhs.get_y() == rhs.get_y();
}
...
std::ostream& operator<<(std::ostream& os, const my_custom_vertex& v) noexcept
{
  os << graphviz_encode(v.get_name()) << ","
     << graphviz_encode(v.get_description()) << "," << v.get_x() << ","
     << v.get_y();
  return os;
}
...
std::istream& operator>>(std::istream& is, my_custom_vertex& v)
{
  ...
  my_custom_vertex new_vertex(graphviz_decode(w[0]), graphviz_decode(w[1]),

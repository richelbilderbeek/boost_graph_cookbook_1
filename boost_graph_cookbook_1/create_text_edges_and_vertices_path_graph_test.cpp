...
BOOST_AUTO_TEST_CASE(
  create_text_edges_and_vertices_path_graph_one_vertex_too_few)
{
...
{
  // Create the .dot and .svg of the 'create_text_edges_and_vertices_path_graph'
  // chapter for (const int n: {3,4, 5} )
  const std::vector<std::string> vertex_names = { "A", "B", "C", "D" };
  const std::vector<std::string> edge_names = { "1", "2", "3" };
  const auto g
    = create_text_edges_and_vertices_path_graph(edge_names, vertex_names);
  const std::string base_filename{
    "create_text_edges_and_vertices_path_graph_4"
  };

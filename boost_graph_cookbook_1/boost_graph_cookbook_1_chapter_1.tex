...
\paragraph{High verbosity}\index{verbose}
...
\paragraph{Repetitiveness}\index{repetitive}
...
\subsection{Coding style}\index{Coding style}
...
computers.
Next to this, it makes code even shorter then just C++11 \index{C++11}.

\paragraph{Coding standard}\index{Coding standard}

I use the coding style from the Core C++ Guidelines \index{Core C++ Guidelines}.
...
\paragraph{No comments in code}\index{Comments}
...
It is good to write generic code \index{Generic code}.
In this tutorial, however, I have chosen my functions to have 
no templated arguments \index{No templates} for conciseness and readability.
...
\paragraph{Long function names}\index{Long function names}
...
\paragraph{Use of auto}\index{auto}
...
same return type.
When C++17 \index{C++17} gets accessible, 
I will use \verb;decltype(auto); \index{decltype}
If you really want to know a type, you can use the \verb;get_type_name; function
(chapter \ref{subsec:get_type_name})

\subparagraph{Explicit use of namespaces}\index{Namespaces}
...
\paragraph{Use of STL algorithms}\index{STL}
...
\paragraph{Tested to work}\index{Test}
...
\paragraph{Availability}\index{Download}
...
\subsection{License}\index{License}
...
\subsection{Acknowledgements}\index{Acknowledgements}

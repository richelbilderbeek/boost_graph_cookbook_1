...
BOOST_AUTO_TEST_CASE(
  add_edge_between_custom_vertices_where_first_does_not_exist)
{
  auto g = create_empty_undirected_custom_vertices_graph();
  const my_custom_vertex va("Bert");
  const my_custom_vertex vb("Ernie");
  ...
  add_custom_vertex(vb, g);
  BOOST_CHECK_THROW(
    add_edge_between_custom_vertices(vc, vb, g), std::invalid_argument);
}

BOOST_AUTO_TEST_CASE(
  add_edge_between_custom_vertices_where_second_does_not_exist)
{
  auto g = create_empty_undirected_custom_vertices_graph();
  const my_custom_vertex va("Bert");
  const my_custom_vertex vb("Ernie");
  ...
  add_custom_vertex(vb, g);
  BOOST_CHECK_THROW(
    add_edge_between_custom_vertices(va, vc, g), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(add_edge_between_custom_vertices_where_edge_already_exists)
{
  ...
  add_edge_between_custom_vertices(va, vb, g);
  BOOST_CHECK_THROW(
    add_edge_between_custom_vertices(va, vb, g), std::invalid_argument);
}

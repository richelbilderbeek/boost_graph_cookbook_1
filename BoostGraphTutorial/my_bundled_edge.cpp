...
{
  {
    const my_bundled_edge in("AB","cd",1.0,2.0);
    std::stringstream s;
    s << in;
    my_bundled_edge out;
    s >> out;
  ...
  {
    const my_bundled_edge a("AB","cd",1.0,2.0);
    const my_bundled_edge b("C","d",3.0,4.0);
    std::stringstream s;
    s << a << " " << b;
    my_bundled_edge c;
    my_bundled_edge d;
...
std::istream& operator>>(std::istream& is, my_bundled_edge& v) noexcept
{
  std::string line;
  is >> line;
  const auto w = seperate_string(line,',');
  if (w.size() != 4) { v = my_bundled_edge(); return is; }
  assert(w.size() == 4);
  v.m_name = w[0];
  v.m_description = w[1];

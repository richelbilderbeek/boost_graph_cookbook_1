...
void create_all_direct_neighbour_custom_and_selectable_edges_and_vertices_subgraphs_test() noexcept
{
  //K2
  {
    const auto v = create_all_direct_neighbour_custom_and_selectable_edges_and_vertices_subgraphs(create_custom_and_selectable_edges_and_vertices_k2_graph());
    assert(v.size() == 2);
    for (const auto g: v)
    {
  ...
  //K3
  {
    const auto v = create_all_direct_neighbour_custom_and_selectable_edges_and_vertices_subgraphs(create_custom_and_selectable_edges_and_vertices_k3_graph());
    assert(v.size() == 3);
    for (const auto g: v)
    {
  ...
  create_all_direct_neighbour_custom_and_selectable_edges_and_vertices_subgraphs_demo();
}

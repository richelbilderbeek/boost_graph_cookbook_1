...
#include "create_empty_directed_named_and_selectable_vertices_graph_demo.impl"
...
void create_empty_directed_named_and_selectable_vertices_graph_test() noexcept
{
  ...
  {
    auto g = create_empty_directed_named_and_selectable_vertices_graph<int>();
    const auto vip = get_vertex_iterators(g);
    assert(vip.first == vip.second);
    const auto vds = get_vertex_descriptors(g);
    assert(vds.empty());
    ...
    assert(boost::num_edges(g) == 0);
    assert(boost::num_vertices(g) == 0);
    add_named_and_selectable_vertex(42,false,g);
    assert(boost::num_edges(g) == 0);
    assert(boost::num_vertices(g) == 1);
    add_edge(g);
    assert(boost::num_edges(g) == 1);
